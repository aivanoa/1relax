...
$APPLICATION->SetTitle("Подробно о салоне");



    
global $APPLICATION;
$cookie = $APPLICATION->get_cookie("PROMO");
if($cookie=="")
{
    $cookie = md5(time().rand(0,100));
    $APPLICATION->set_cookie("PROMO", $cookie, time()+3600*24);
}
$_REQUEST['s'] = $cookie;
$_REQUEST['CITY'] = $GLOBALS["arCity"]['ID'];
$_REQUEST['UTM'] = getUtm();
...
?> 
<?
//ќбработка кликов на салон
//require($_SERVER["DOCUMENT_ROOT"]."/salons/click.php");

$el = CIBlockElement::GetList(array(),array("CODE"=>$_REQUEST["ELEMENT_CODE"]),false,false,array("PROPERTY_REKLAMA_OFF"))->Fetch();
if($el["PROPERTY_REKLAMA_OFF_VALUE"]!="Y"):
?>
    <script>
        $(function(){
            setTimeout(function() {
...
<?$APPLICATION->IncludeComponent("architect:news.detail", "salon", array(
	"IBLOCK_TYPE" => "content",
	"IBLOCK_ID" => "1",
	"ELEMENT_ID" => "",
...
preg_match("/<TITLE>(.*)<\/TITLE>/im", $componentOut, $regs);
