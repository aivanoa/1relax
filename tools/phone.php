...
if (!function_exists('isPhoneAvailable')) {
    function isPhoneAvailable( $phone, $ID = 1 ){
        return (CIBlockElement::GetList(Array("ID"=>"ASC"), array("=PROPERTY_PHONEAUTH" => $phone, "!ID" => $ID), false, false)->SelectedRowsCount() > 0) ? false : true;
    }
}


/*
$res =   CIBlockElement::GetList(array(),array("IBLOCK_ID"=>1),false,false,array("PROPERTY_PHONE","ID"));

while($arSalon = $res->GetNext())
{
   $ar = explode(",",$arSalon["PROPERTY_PHONE_VALUE"]);


   $arSalon["PROPERTY_PHONE_VALUE"] = $ar[0];
  //CIBlockElement::SetPropertyValuesEx($arSalon["ID"], 1, array("PHONEAUTH" => phone($arSalon["PROPERTY_PHONE_VALUE"])));
echo phone($arSalon["PROPERTY_PHONE_VALUE"])."<br>";
